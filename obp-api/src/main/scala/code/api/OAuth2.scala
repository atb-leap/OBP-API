...
object OAuth2Login extends RestHelper with MdcLoggable {
  ...
  def getUser(cc: CallContext): (Box[User], Option[CallContext]) = {
    APIUtil.getPropsAsBoolValue("allow_oauth2_login", true) match {
      case true =>
        ...
        } else if (ATB.isIssuer(value)) {
          ATB.applyRules(value, cc)
        } else {
  ...
  object ATB extends OAuth2Util {
    ...
    override def urlOfJwkSets: Box[String] = checkUrlOfJwkSets(identityProvider = atb)
    def isIssuer(jwt: String): Boolean = isIssuer(jwtToken=jwt, identityProvider = atb)
  }
